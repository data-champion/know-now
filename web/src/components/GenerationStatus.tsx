...
export function GenerationStatus() {
  ...
  if (loading) return <div className="kn-loading" aria-live="polite">Loading generation status...</div>;
  if (error) return <div className="kn-error" role="alert">Failed to load status: {error.message}</div>;
  if (!data) return null;

  return (
    <section className="kn-gen-status" aria-label="Generation status">
